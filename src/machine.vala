...
private abstract class VirtualPC.Machine: VirtualPC.CollectionItem {
    const uint AUTOSAVE_TIMEOUT = 60; // seconds

    public VirtualPC.CollectionSource source;
    public VirtualPC.BoxConfig config;
    ...
    public MachineState state { get { return _state; }
        protected set {
            ...
            if (display != null && !stay_on_display &&
                ...
                window.set_state (VirtualPC.UIState.COLLECTION);
            }
        }
    }
    ...
    protected void show_display () {
        ...
        case VirtualPC.UIState.CREDS:
            window.set_state (VirtualPC.UIState.DISPLAY);
            show_display ();
            break;

        case VirtualPC.UIState.DISPLAY:
        ...
        case VirtualPC.UIState.PROPERTIES:
            var widget = display.get_display (0);
            widget_remove (widget);
            ...
            disconnected_id = _display.disconnected.connect ((failed) => {
                ...
                if (window.ui_state == UIState.CREDS || window.ui_state == UIState.DISPLAY) {
                    ...
                    if (failed) {
                        ...
                        window.display_toast (new VirtualPC.Toast (message));
                    }
                }
    ...
    protected Machine (VirtualPC.CollectionSource source, string name, string? uuid = null) {
        this.name = name;
        this.source = source;
    ...
    private string get_screenshot_filename () throws VirtualPC.Error {
        if (config.uuid == null)
            throw new VirtualPC.Error.INVALID ("no uuid, cannot build screenshot filename");

        return VirtualPC.get_screenshot_filename (config.uuid);
    }
    ...
    // screenshots can have its use in case of an abnormal lingmo-virtualpc
    // termination.
    ...
    private void set_screenshot (Gdk.Pixbuf? large_screenshot, bool save) {
        if (large_screenshot != null) {
            ...
            /* We don't accept black or almost-black screenshots as they are
               ...
               and can easily be mistaken for turned off virtualpc.

               The number 100 is somewhat arbitrary, picked to not allow the lingmo 3
    ...
    private async void try_connect_display (ConnectFlags flags = ConnectFlags.NONE) {
        try {
        ...
        } catch (VirtualPC.Error.RESTORE_FAILED e) {
            ...
            window.display_toast (new VirtualPC.Toast () {
                message = message,
                action = _("Restart"),
                undo_func = (owned) restart_func,
                dismiss_func = (owned) dismiss_func
            });
        } catch (VirtualPC.Error.START_FAILED e) {
            ...
            if (this is LibvirtMachine) {
                ...
                window.display_toast (new VirtualPC.Toast (msg));
            }
