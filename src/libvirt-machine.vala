...
private class VirtualPC.LibvirtMachine: VirtualPC.Machine {
    ...
    public bool run_in_bg { get; set; } // If true, machine will never be paused automatically by VirtualPC.
    ...
    private Display? create_display () throws VirtualPC.Error {
        ...
        default:
            throw new VirtualPC.Error.INVALID ("unsupported display of type " + type);
        }
    }
    ...
    public override void delete (bool by_user = true) {
        ...
        if (by_user) {
            ...
            App.app.async_launcher.launch.begin ( () => {
                ...
                // Remove any images controlled by virtualpc
    ...
    public async void start (Machine.ConnectFlags flags, Cancellable? cancellable = null) throws GLib.Error {
        ...
        if (state == MachineState.PAUSED)
            ...
            try {
                ...
                if (restore)
                    throw new VirtualPC.Error.RESTORE_FAILED (error.message);
                else
                    throw new VirtualPC.Error.START_FAILED (error.message);
            }
