...
[GtkTemplate (ui = "/org/lingmo/VirtualPC/ui/app-window.ui")]
private class VirtualPC.AppWindow: Hdy.ApplicationWindow, VirtualPC.UI {
    ...
    [GtkChild]
    public unowned VirtualPC.ToastOverlay toast_overlay;
    [GtkChild]
    public unowned DisplayPage display_page;
    [GtkChild]
    public unowned Hdy.StatusPage empty_virtualpc;
    [GtkChild]
    ...
    public AppWindow (Gtk.Application app) {
        Object (application:  app,
                title:        _("VirtualPC"),
                // Can't be set from template: https://bugzilla.lingmo.org/show_bug.cgi?id=754426#c14
                show_menubar: false);

        settings = new GLib.Settings ("org.lingmo.virtualpc");

        Gtk.Window.set_default_icon_name (Config.APPLICATION_ID);
        Hdy.StyleManager.get_default ().color_scheme = PREFER_DARK;

        var provider = VirtualPC.load_css ("gtk-style.css");
        ...
        if (app.application_id == "org.lingmo.VirtualPCDevel") {
    ...
    private void on_collection_changed () {
        ...
        if (collection_is_empty)
            below_bin.visible_child = empty_virtualpc;
        else
    ...
    private void ui_state_changed () {
        ...
        case UIState.COLLECTION:
            ...
            topbar.status = _("VirtualPC");
            var a11y = get_accessible ();
            a11y.accessible_name = _("VirtualPC");
    ...
    public void show_vm_assistant (string? path = null) {
        new VirtualPC.Assistant (this, path).present ();
    }

    public void show_welcome_tutorial () {
        if (first_run) {
            new VirtualPC.WelcomeTutorial (this).run ();
    ...
    public void connect_to (Machine machine) {
        ...
        got_error_id = machine.got_error.connect ((message) => {
            display_toast (new VirtualPC.Toast (message));
        });
