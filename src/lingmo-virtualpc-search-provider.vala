...
[DBus (name = "org.lingmo.Shell.SearchProvider2")]
public class VirtualPC.SearchProvider: Object {
    private SearchProviderApp app;
    private bool loading;
    public bool loaded { get; set; }
    private HashTable<string, BoxConfig> virtualpc;
    private uint next_id;

    public SearchProvider (SearchProviderApp app) {
        this.app = app;
        virtualpc = new HashTable<string, BoxConfig> (str_hash, str_equal);
    }
    ...
    private async string[] search (owned string[] terms) {
        ...
        foreach (var box in virtualpc.get_values ()) {
    ...
    public async HashTable<string, Variant>[] get_metas (owned string[] ids) throws GLib.Error {
        ...
        foreach (var id in ids) {
            ...
            var file = File.new_for_path (VirtualPC.get_screenshot_filename (box.uuid));
            FileInfo? info = null;
            try {
            ...
            if (info != null) {
                ...
                meta.insert ("gicon", new Variant.string (new ThemedIcon ("lingmo-virtualpc").to_string ()));
            }
        }
    ...
    public void ActivateResult (string search_id, string[] terms, uint32 timestamp) throws GLib.Error {
        ...
        var box = virtualpc.lookup (search_id);
        if (box == null) {
        ...
        try {
            var cmd = "lingmo-virtualpc --open-uuid " + uuid;
            if (!Process.spawn_command_line_async (cmd))
                stderr.printf ("Failed to launch VirtualPC with uuid '%s'\n", uuid);
...
public class VirtualPC.SearchProviderApp: GLib.Application {
    public SearchProviderApp () {
        Object (application_id: "org.lingmo.VirtualPC.SearchProvider",
    ...
    public override void startup () {
        if (Environment.get_variable ("VIRTUALPC_SEARCH_PROVIDER_PERSIST") != null)
            hold ();
        base.startup ();
    }
}
...
int main () {
    return new VirtualPC.SearchProviderApp ().run ();
}
