...
private abstract class VirtualPC.Broker : GLib.Object {
...
private class VirtualPC.App: Gtk.Application {
    public static App app;

    public const string DEFAULT_SOURCE_NAME = "QEMU Session";

    private List<VirtualPC.AppWindow> windows;
    ...
    public App () {
        ...
        resource_base_path = "/org/lingmo/VirtualPC";

        app = this;
        async_launcher = AsyncLauncher.get_default ();
        windows = new List<VirtualPC.AppWindow> ();
        ...
        action.activate.connect (() => {
            try {
                Gtk.show_uri_on_window (main_window, "help:lingmo-virtualpc", Gtk.get_current_event_time ());
            } catch (GLib.Error e) {
        ...
        action.activate.connect (() => {
            string[] authors = {
                ...
                "Zeeshan Ali (Khattak) <zeeshanak@lingmo.org>"
            };
            string[] artists = {
                "Allan Day <aday@lingmo.org>",
            ...
            Gtk.show_about_dialog (main_window,
                                   ...
                                   "comments", _("A simple LINGMO application to access virtual systems"),
                                   "copyright", "\xc2\xa9 2011-2018 Red Hat, Inc.",
                                   "license-type", Gtk.License.LGPL_2_1,
                                   "program-name", _("VirtualPC") + Config.NAME_SUFFIX,
    ...
    public bool open_uuid (string uuid) {
        ...
        for (int i = 0 ; i < collection.length ; i++) {
            ...
            if (!(item is VirtualPC.Machine))
                continue;
            var machine = item as VirtualPC.Machine;
    ...
    private void notify_vm_is_running_in_background (LibvirtMachine machine) {
        ...
        send_notification ("lingmo-virtualpc-run-in-bg-%s" + machine.name, notification);
    }
    ...
    private void suspend_machines () {
        // if we are not the main VirtualPC instance, 'collection' won't
        // be set as it's created in GtkApplication::startup()
        if (collection == null)
            return;

        debug ("Suspending running virtualpc");
        ...
        // the VMs during VirtualPC shutdown.
        ...
        debug ("Running virtualpc suspended");
    }
    ...
    public void delete_machines_undoable (owned List<CollectionItem> items,
        ...
        main_window.display_toast (new VirtualPC.Toast () {
            message = msg,
            action = _("Undo"),
    ...
    public AppWindow add_new_window () {
        var window = new VirtualPC.AppWindow (this);
    ...
    public new void inhibit (Gtk.Window? window = main_window, Gtk.ApplicationInhibitFlags? flags = null, string? reason = null) {
        if (reason == null) {
            reason = _("VirtualPC is doing something");
        }
