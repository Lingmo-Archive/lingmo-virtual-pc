...
[GtkTemplate (ui = "/org/lingmo/VirtualPC/ui/collection-toolbar.ui")]
private class VirtualPC.CollectionToolbar: Hdy.HeaderBar {
    [GtkChild]
    ...
    public void setup_ui (AppWindow window) {
        ...
        var builder = new Builder.from_resource ("/org/lingmo/VirtualPC/ui/menus.ui");
    ...
    private void open_filechooser () {
        ...
        if (file_chooser.run () == Gtk.ResponseType.ACCEPT) {
            new VirtualPC.Assistant (window, file_chooser.get_filename ()).present ();
        }
    ...
    private void open_downloads_dialog () {
        new VirtualPC.OsDownloader (window).present ();
        new_vm_popover.popdown ();
    }
