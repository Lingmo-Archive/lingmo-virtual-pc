...
private errordomain VirtualPC.VMConfiguratorError {
    NO_GUEST_CAPS,
}

private class VirtualPC.VMConfigurator {
    private const string VIRTUALPC_NS = "virtualpc";
    /* This should not reference PACKAGE_URL, which could change in the
       future, but this is effectively part of virtualpc XML API */
    private const string VIRTUALPC_NS_URI = "https://wiki.lingmo.org/Apps/VirtualPC";
    private const string VIRTUALPC_OLD_NS_URI = "http://live.lingmo.org/VirtualPC/";
    ...
    private const string VIRTUALPC_XML = "<lingmo-virtualpc>%s</lingmo-virtualpc>";
    ...
    public static bool is_virtualpc_installed (Domain domain) {
    ...
    public static async void update_existing_domain (Domain          domain,
        ...
        if (!virtualpc_created_domain (domain))
            return;

        if (virtualpc_edited_domain (domain)) {
            return;
        }
    ...
    private static void update_domain_devices (Domain domain) throws GLib.Error {
        ...
        foreach (var device in domain.get_devices ()) {
            ...
            // Fix broken audio. FIXME. See https://gitlab.gnome.org/GNOME/gnome-virtualpc/-/issues/738
    ...
    public static string? get_custom_xml_node (Domain domain, string node_name) {
        var ns_uri = VIRTUALPC_NS_URI;
        var xml = domain.get_custom_xml (ns_uri);
        if (xml == null) {
            ns_uri = VIRTUALPC_OLD_NS_URI;
        ...
        if (xml != null) {
            ...
            var node = reader.expand ();
            if (node != null) {
                // Newer configurations have nodes of interest under a toplevel 'lingmo-virtualpc' element
                if (node->name == "lingmo-virtualpc")
                    node = node->children;
    ...
    private static bool virtualpc_created_domain (Domain domain) {
        var xml = domain.get_custom_xml (VIRTUALPC_NS_URI);
        if (xml == null)
            xml = domain.get_custom_xml (VIRTUALPC_OLD_NS_URI);

        return (xml != null);
    }

    private static bool virtualpc_edited_domain (Domain domain) {
    ...
    private static void update_custom_xml (Domain domain,
        ...
        custom_xml = VIRTUALPC_XML.printf (custom_xml);
        try {
            domain.set_custom_xml (custom_xml, VIRTUALPC_NS, VIRTUALPC_NS_URI);
