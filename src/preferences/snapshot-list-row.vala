...
[GtkTemplate (ui = "/org/lingmo/VirtualPC/ui/preferences/snapshot-list-row.ui")]
private class VirtualPC.SnapshotListRow : Hdy.ActionRow {
    public signal void display_toast (VirtualPC.Toast toast);
    ...
    private VirtualPC.LibvirtMachine machine;
    ...
    public SnapshotListRow (GVir.DomainSnapshot snapshot,
        ...
        try {
            ...
            display_toast (new VirtualPC.Toast (e.message));
        }
    }
    ...
    private void on_save_name_button_clicked () {
        ...
        try {
            ...
            // Translators: %s is the reason why VirtualPC failed to rename the snapshot.
            display_toast (new VirtualPC.Toast (_("Failed to rename snapshot: %s").printf (e.message)));
        }
    }
    ...
    private void on_revert_button_clicked () {
        ...
        snapshot.revert_to_async.begin (0, null, (obj, res) => {
            try {
                ...
                // Translators: %s is the reason why VirtualPC failed to apply the snapshot.
                display_toast (new VirtualPC.Toast (_("Failed to revert to snapshot: %s").printf (e.message)));
            }
    ...
    private void on_delete_button_clicked () {
        ...
        display_toast (new VirtualPC.Toast () {
            message = message,
            action = _("Undo"),
