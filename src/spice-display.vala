...
private class VirtualPC.SpiceDisplay: VirtualPC.Display {
    ...
    public SpiceDisplay (Machine machine, BoxConfig config, string host, int port, int tls_port = 0)
        ...
        session.cert_subject = GLib.Environment.get_variable ("VIRTUALPC_SPICE_HOST_SUBJECT");
    ...
    public override Gdk.Pixbuf? get_pixbuf (int n) {
         ...
         * VirtualPC to crash when calling spice_display_get_pixbuf ();
    ...
    private GLib.GenericArray<Spice.UsbDevice> get_usb_devices (UsbDeviceManager manager) {
        ...
        if (Environment.get_variable ("VIRTUALPC_USB_REDIR_ALL") != null)
            return devs;
    ...
    public GLib.ListStore get_usb_devices_model () {
        GLib.ListStore model = new GLib.ListStore (typeof (VirtualPC.UsbDevice));
        ...
        for (int i = 0; i < devs.length; i++) {
            ...
            var usb_device = new VirtualPC.UsbDevice () {
...
private class VirtualPC.SpiceChannelHandler : GLib.Object {
