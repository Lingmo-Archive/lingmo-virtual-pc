...
namespace VirtualPC {
    ...
    public Gtk.CssProvider load_css (string css) {
        ...
        try {
            var file = File.new_for_uri("resource:///org/lingmo/VirtualPC/" + css);
    ...
    public Gdk.Pixbuf load_asset (string asset) throws GLib.Error {
        return new Gdk.Pixbuf.from_resource ("/org/lingmo/VirtualPC/icons/" + asset);
    }

    public Gtk.Builder load_ui (string ui) {
        var builder = new Gtk.Builder ();
        try {
            builder.add_from_resource ("/org/lingmo/VirtualPC/ui/".concat (ui, null));
        } catch (GLib.Error e) {
    ...
    public string? extract_xpath (string xmldoc, string xpath, bool required = false) throws VirtualPC.Error {
        var parser = new ParserCtxt ();
        var doc = parser.read_doc (xmldoc, "doc.xml");

        if (doc == null)
            throw new VirtualPC.Error.INVALID ("Can't parse XML doc");

        var ctxt = new XPath.Context (doc);
        var obj = ctxt.eval (xpath);
        if (obj == null || obj->stringval == null) {
            if (required)
                throw new VirtualPC.Error.INVALID ("Failed to extract xpath " + xpath);
            else
                return null;
        }
    ...
    public async GLib.List<Osinfo.Media>? get_recommended_downloads () {
        return yield parse_recommended_downloads_file (
            "resource:///org/lingmo/VirtualPC/recommended-downloads.xml");
    }
    ...
    public async bool check_libvirt_kvm () {
        try {
            ...
            if (index < 0)
                throw new VirtualPC.Error.INVALID ("Unexpected output while running command '%s %s'", argv[0], argv[1]);
    ...
    public async bool check_storage_pool (out string diagnosis) {
        ...
        try {
            ...
            if (index < 0)
                throw new VirtualPC.Error.INVALID ("Unexpected output while running command '%s %s %s'", argv[0], argv[1], argv[2]);

            standard_output = standard_output.substring (index);
            pool_path = extract_xpath (standard_output, "string(/pool[@type='dir']/target/path)");
        } catch (GLib.Error error) {
            debug (error.message);
            diagnosis = _("Could not get “lingmo-virtualpc” storage pool information from libvirt. Make sure “virsh -c qemu:///session pool-dumpxml lingmo-virtualpc” is working.");
            return false;
        }

        if (!FileUtils.test (pool_path, FileTest.EXISTS)) {
            diagnosis = _("%s is known to libvirt as LINGMO VirtualPC’s storage pool but this directory does not exist").printf (pool_path);
            return false;
        }
        if (!FileUtils.test (pool_path, FileTest.IS_DIR)) {
            diagnosis = _("%s is known to libvirt as LINGMO VirtualPC’s storage pool but is not a directory").printf (pool_path);
            return false;
        }
        if (Posix.access (pool_path, Posix.R_OK | Posix.W_OK | Posix.X_OK) != 0) {
            diagnosis = _("%s is known to libvirt as LINGMO VirtualPC’s storage pool but is not user-readable/writable").printf (pool_path);
            return false;
        }
    ...
    public string get_timezone_from_systemd () throws GLib.Error {
        ...
        if (timedate.timezone == null)
            throw new VirtualPC.Error.INVALID ("Failed to get timezone from systemd");
    ...
    public string get_timezone_from_linux () throws GLib.Error {
        ...
        if (!file.query_exists ())
            throw new VirtualPC.Error.INVALID ("Timezone file not found in expected location '%s'", TZ_FILE);
        ...
        if (target == null)
            throw new VirtualPC.Error.INVALID ("Timezone file '%s' is expected to be a symlink", TZ_FILE);

        var tokens = target.split ("zoneinfo/");
        if (tokens == null || tokens.length < 2)
            throw new VirtualPC.Error.INVALID ("Timezone file in unexpected location '%s'", target);
    ...
    private DBusProxy? create_lingmo_settings_dbus_proxy () {
        ...
        try {
            ...
            debug ("Failed to launch org.lingmo.Settings. Fallback to org.lingmo.ControlCenter");
        }
        ...
        try {
            proxy = new DBusProxy.for_bus_sync (BusType.SESSION,
                                                DBusProxyFlags.NONE,
                                                null,
                                                "org.lingmo.ControlCenter",
                                                "/org/lingmo/ControlCenter",
                                                "org.gtk.Actions");
        } catch (GLib.Error error) {
            debug ("Failed to launch org.lingmo.ControlCenter");
        }
    ...
    public void open_permission_settings () {
        try {
            var proxy = create_lingmo_settings_dbus_proxy ();
            if (proxy == null)
                throw new GLib.IOError.FAILED ("Couldn't create DBusProxy for LINGMO Settings");
            ...
            warning ("Failed to launch lingmo-control-center: %s", error.message);
        }
    }
}
