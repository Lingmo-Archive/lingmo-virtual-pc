Feature: Snapshots
  ...
  Background:
    * Make sure that lingmo-virtualpc is running
  ...
  Scenario: Create snapshots and revert to them
    ...
    * Create snapshot "network down" from machine "Core-5"
    When "network down" is visible with command "virsh snapshot-current virtualpc-unknown |grep description"
    When Cannot ping "Core-5"
    * Revert machine "Core-5" to state "working network"
    When "working network" is visible with command "virsh snapshot-current virtualpc-unknown |grep description"
    When Ping "Core-5"
  ...
  Scenario: Delete snapshots
    ...
    * Delete machines "Core-5" snapshot "network down"
    Then "error: domain 'virtualpc-unknown' has no current snapshot" is visible with command "virsh snapshot-current virtualpc-unknown"
