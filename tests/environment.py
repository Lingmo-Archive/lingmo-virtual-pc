...
def do_backup():
    ...
    try:
        call("mkdir ~/virtualpc_backup", shell=True)

        print("** Backing up session machines")

        # First ensure all virtualpc are shutdown
        ...
        call("for i in $(virsh list --all|tail -n +3|head -n -1|sed -e 's/^ \(-\|[0-9]\+\) *//'|cut -d' ' -f1); do " +
             "    virsh dumpxml $i > ~/virtualpc_backup/$i.xml; done", shell=True, stdout=f)

        # move disk images
        call("mkdir ~/virtualpc_backup/images", shell=True)
        call("mv ~/.local/share/lingmo-virtualpc/images/* ~/virtualpc_backup/images/ || true", shell=True, stderr=f)

        # Backup snapshots
        call("mkdir ~/virtualpc_backup/snapshot", shell=True)
        call("cp -R ~/.config/libvirt/qemu/snapshot/* ~/virtualpc_backup/snapshot/ || true", shell=True, stderr=f)
        ...
        # move save states
        call("mkdir ~/virtualpc_backup/save", shell=True)
        call("mv ~/.config/libvirt/qemu/save/* ~/virtualpc_backup/save/ || true", shell=True, stderr=f)

        # move all sources
        print("** Backing up all sources")

        call("mkdir ~/virtualpc_backup/sources", shell=True)
        call("mv ~/.cache/lingmo-virtualpc/sources/* ~/virtualpc_backup/sources/ || true", shell=True, stderr=f)

        # create marker
        call('touch /tmp/virtualpc_backup', shell=True)
        print("* Done\n")

    except Exception as e:
        print("Error in backup: %s" % e.message)

    # Undefine all virtualpc
    call("for i in $(virsh list --all|tail -n +3|head -n -1|sed -e 's/^ \(-\|[0-9]\+\) *//'|cut -d' ' -f1); do " +
         "    virsh undefine --managed-save $i; done", shell=True, stdout=f)
    f.close()

def do_restore():
    if not os.path.isfile('/tmp/virtualpc_backup'):
        return

    f = open(os.devnull, "w")

    print("** Restoring VirtualPC backup")
    # move images back
    call("mv ~/virtualpc_backup/images/* ~/.local/share/lingmo-virtualpc/images/", shell=True, stderr=f)

    # move save states back
    #call("mkdir ~/.config/libvirt/qemu/save/", shell=True, stdout=f, stderr=f)
    call("mv ~/virtualpc_backup/save/* ~/.config/libvirt/qemu/save/", shell=True, stdout=f, stderr=f)

    # import machines
    call("for i in $(ls ~/virtualpc_backup |grep xml); do virsh define ~/virtualpc_backup/$i; done", shell=True, stdout=f)
    ...
    call("export IFS='_'; cd ~/virtualpc_backup/snapshot/; \
          for i in * ; do \
              for s in $i/*.xml; \
                  do virsh snapshot-create $i ~/virtualpc_backup/snapshot/$s; \
              done; \
              virsh start $i; \
              for s in $i/*.xml; \
                  do virsh snapshot-create $i ~/virtualpc_backup/snapshot/$s; \
    ...
    call("mv ~/virtualpc_backup/sources/* ~/.cache/lingmo-virtualpc/sources/", shell=True, stdout=f)

    # delete marker
    call("rm -rf /tmp/virtualpc_backup", shell=True, stdout=f)

    # delete backup
    call("rm -rf ~/virtualpc_backup", shell=True, stdout=f)
...
def before_all(context):
    ...
    try:
        if not os.path.isfile('/tmp/virtualpc_configured'):
            print("** Turning off lingmo idle")
            if call("gsettings set org.lingmo.desktop.session idle-delay 0", shell=True) == 0:
                print("PASS\n")
            else:
            ...
            call('touch /tmp/virtualpc_configured', shell=True)
        ...
        context.app_class = App('lingmo-virtualpc')
    ...
    try:
        if step.status == 'failed' and hasattr(context, "embed"):
            ...
            os.system("dbus-send --print-reply --session --type=method_call " +
                      "--dest='org.lingmo.Shell.Screenshot' " +
                      "'/org/lingmo/Shell/Screenshot' " +
                      "org.lingmo.Shell.Screenshot.Screenshot " +
...
def after_tag(context, tag):
    if 'express_install' in tag:
        ...
        call('rm -rf ~/.cache/lingmo-virtualpc/unattended', shell=True)
...
def after_scenario(context, scenario):
    ...
    Kill lingmo-virtualpc (in order to make this reliable we send sigkill)
    """
    ...
    try:
        ...
        for tag in scenario.tags:
            if 'system_broker' in tag:
                call("rm -rf ~/.cache/lingmo-virtualpc/sources/qemu____system", shell=True)
        ...
        if hasattr(context, "embed"):
            os.system("journalctl /usr/bin/lingmo-session --no-pager -o cat --since='%s'> /tmp/journal-session.log" \
        ...
        # Remove all volumes
        call("for i in $(virsh vol-list lingmo-virtualpc|tail -n +3|head -n -1|sed 's/^ //'|cut -d' ' -f1); do " +
             "    virsh vol-delete $i lingmo-virtualpc; done", shell=True, stdout=f)

        # Remove all save states
        call("rm -rf ~/.config/libvirt/qemu/save/*", shell=True, stderr=f)

        # Remove all sources
        call("rm -rf ~/.cache/lingmo-virtualpc/sources/*", shell=True, stderr=f)
