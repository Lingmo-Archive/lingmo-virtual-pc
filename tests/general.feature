Feature: General
  ...
  @quit_via_panel
  Scenario: Quit VirtualPC via super menu
    * Select "Quit" from supermenu
    Then VirtualPC are not running

  @quit_via_shortcut
  Scenario: Quit VirtualPC via shortcut
    * Select "Quit" from supermenu
    Then VirtualPC are not running

  @no_virtualpc
  Scenario: No virtualpc installed
  ...
  Scenario: Customize machine before installation
    ...
    * Press "Create"
    * Wait for "sleep 10" end
    Then "65536 KiB" is visible with command "DOM=$(virsh list |grep virtualpc |awk {'print $1'}); virsh dominfo $DOM"
